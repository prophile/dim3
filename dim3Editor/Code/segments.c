...
void segment_reset_texture_uv_get_scale(segment_type *seg,float *txt_scale_x,float *txt_scale_y)
{
...
float segment_reset_texture_uv_reduce_txtoff(float txtoff)
{
	while (txtoff>=1) {
		txtoff-=1;
	}
...
void segment_reset_texture_uv_get_factor_size(int sx,int sz,int ptsz,int *x,int *z,float *xtoff,float *ztoff,float *xfact,float *zfact,float txt_scale_x,float txt_scale_y)
{
	int			lft,rgt,top,bot;
	float		ltxtx,rtxtx,ltxtz,rtxtz;

	poly_find_bound_rect(ptsz,x,z,&lft,&rgt,&top,&bot);
	...
	*xtoff=segment_reset_texture_uv_reduce_txtoff(ltxtx);
	*xfact=rtxtx-ltxtx;
	
	*ztoff=segment_reset_texture_uv_reduce_txtoff(ltxtz);
	*zfact=rtxtz-ltxtz;
}

void segment_reset_texture_uvs(segment_type *seg)
{
	int					k;
	...
	segment_reset_texture_uv_get_scale(seg,&txt_scale_x,&txt_scale_y);
	...
	switch (seg->type) {
		...
		case sg_floor:
		case sg_ceiling:
			segment_reset_texture_uv_get_factor_size(portal->x,portal->z,seg->data.fc.ptsz,seg->data.fc.x,seg->data.fc.z,&seg->x_txtoff,&seg->y_txtoff,&seg->x_txtfact,&seg->y_txtfact,txt_scale_x,txt_scale_y);
			break;
		case sg_liquid:		
