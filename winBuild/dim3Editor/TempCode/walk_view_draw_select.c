...
void walk_view_get_ambient_fc_poly(segment_type *seg,int *x,int *z,int *y,float *gx,float *gy,int xadd,int zadd)
{
	...
	edit_view_draw_poly_find_bound_rect(ptsz,seg->data.ambient_fc.x,seg->data.ambient_fc.z,&lft,&rgt,&top,&bot);
	lft=lft*map_enlarge;
	rgt=rgt*map_enlarge;
	top=top*map_enlarge;
	bot=bot*map_enlarge;
	...
	for (n=(sel_count-1);n>=0;n--) {
		...
		switch (type) {
			...
			case primitive_piece:
				...
				primitive_uid=seg->primitive_uid;
				
				seg2=map.segments;
					
				for (k=0;k!=map.nsegment;k++) {
					if ((seg2->primitive_uid==primitive_uid) && (k!=index)) walk_view_draw_select_segment(seg2,1);
					seg2++;
				}
			...
			case spot_piece:
				...
				if (!walk_view_model_draw_select(&map.spots[index].pos,map.spots[index].display_model)) {
			...
			case scenery_piece:
				...
				if (!walk_view_model_draw_select(&map.sceneries[index].pos,map.sceneries[index].model_name)) {
