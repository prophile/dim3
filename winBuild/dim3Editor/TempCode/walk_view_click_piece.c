...
void walk_view_draw_feedback_sprite(int type,int idx,d3pos *pos,int xadd,int zadd,char *display_model)
{
    int			x,z,y,wid,high;
	...
	if (display_model!=NULL) {
		walk_view_model_click_select_size(display_model,&wid,&high);
    }
...
void walk_view_draw_feedback_map(int rn,bool sel_only)
{
	...
	if (dp_node) {
		...
		for (n=0;n<map.nnode;n++) {
			...
			walk_view_draw_feedback_sprite(node_piece,n,&node->pos,xadd,zadd,NULL);
		}
    ...
    if (dp_object) {
		...
		for (n=0;n<map.nspot;n++) {
			...
			walk_view_draw_feedback_sprite(spot_piece,n,&spot->pos,xadd,zadd,spot->display_model);
		}		
		...
		for (n=0;n<map.nscenery;n++) {
			...
			walk_view_draw_feedback_sprite(scenery_piece,n,&scenery->pos,xadd,zadd,scenery->model_name);
		}		
	}
	...
	if (dp_lightsoundparticle) {
		...
		for (n=0;n<map.nlight;n++) {
			...
			walk_view_draw_feedback_sprite(light_piece,n,&light->pos,xadd,zadd,NULL);
		}
		...
		for (n=0;n<map.nsound;n++) {
			...
			walk_view_draw_feedback_sprite(sound_piece,n,&sound->pos,xadd,zadd,NULL);
		}
		...
		for (n=0;n<map.nparticle;n++) {
			...
			walk_view_draw_feedback_sprite(particle_piece,n,&particle->pos,xadd,zadd,NULL);
		}
    }
}
...
bool walk_view_polygon_click_select(int xorg,int yorg,Point pt)
{
	...
	if ((!main_wind_shift_down()) && (type==segment_piece)) {
		...
		if (map.segments[index].primitive_uid!=-1) {
...
void walk_view_polygon_click_normal(int xorg,int yorg,Point pt,bool dblclick)
{
	...
	if (type==segment_piece) {
		if (map.segments[index].primitive_uid!=-1) type=primitive_piece;
	}
	...
	if (!main_wind_shift_down()) {
		...
		if (type==primitive_piece) {				// if it's a primitive, then let's find the original index to flip
			primitive_index=select_check_primitive_find_index(map.segments[index].primitive_uid);
