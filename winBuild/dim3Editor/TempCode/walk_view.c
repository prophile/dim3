...
void walk_view_setup(bool active,bool full_screen)
{
	...
	if (full_screen) {
		walk_view_box=wbox;
		walk_view_box.top+=(toolbar_high+tab_high);
		walk_view_box.bottom-=(txt_palette_high+info_high);
		walk_view_box.right-=piece_wid;
	}
	else {
		if (!main_wind_rot) {
			walk_view_box.top=wbox.top+(toolbar_high+tab_high+2);
			walk_view_box.bottom=(wbox.top+toolbar_high+tab_high)+((((wbox.bottom-(txt_palette_high+info_high))-(wbox.top+(toolbar_high+tab_high)))/2)-2);
