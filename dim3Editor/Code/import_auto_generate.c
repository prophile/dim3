...
bool map_auto_generate_block_collision(int x,int z,int ex,int ez)
{
...
int map_auto_generate_portal_find_to_left(portal_type *org_portal)
{
	int			n;
	portal_type	*portal;
...
int map_auto_generate_portal_find_to_right(portal_type *org_portal)
{
	int			n;
	portal_type	*portal;
...
int map_auto_generate_portal_find_to_top(portal_type *org_portal)
{
	int			n;
	portal_type	*portal;
...
int map_auto_generate_portal_find_to_bottom(portal_type *org_portal)
{
	int			n;
	portal_type	*portal;
...
void map_auto_generate_move_wall_segment_to_portal(int rn,int seg_idx)
{
	int				lx,lz,rx,rz;
	portal_type		*portal;
	segment_type	*seg;
...
void map_auto_generate_initial_portals(void)
{
	...
	for (i=0;i!=ag_settings.initial_portal_count;i++) {
		...
		while (TRUE) {
			...
			if ((!map_auto_generate_block_collision(x,z,ex,ez)) && (!map_auto_generate_portal_collision(x,z,ex,ez,-1))) break;
		...
		for (n=0;n!=map.nportal;n++) {
			...
			for (k=0;k!=map.nportal;k++) {
				...
				if ((dist>0) && (dist<ag_settings.max_portal_merge_distance)) {
					if (!map_auto_generate_portal_collision((merge_portal->x-merge_add),merge_portal->z,merge_portal->ex,merge_portal->ez,k)) {
				...
				if ((dist>0) && (dist<ag_settings.max_portal_merge_distance)) {
					if (!map_auto_generate_portal_collision(merge_portal->x,merge_portal->z,(merge_portal->ex+merge_add),merge_portal->ez,k)) {
				...
				if ((dist>0) && (dist<ag_settings.max_portal_merge_distance)) {
					if (!map_auto_generate_portal_collision(merge_portal->x,(merge_portal->z-merge_add),merge_portal->ex,merge_portal->ez,k)) {
				...
				if ((dist>0) && (dist<ag_settings.max_portal_merge_distance)) {
					if (!map_auto_generate_portal_collision(merge_portal->x,merge_portal->z,merge_portal->ex,(merge_portal->ez+merge_add),k)) {
...
void map_auto_generate_connect_portals(void)
{
	int				n,k,rn,
	...
	for (n=0;n!=nportal;n++) {
		...
		for (k=0;k!=nportal;k++) {
			...
			if ((map_auto_generate_block_collision(x,z2,ex,ez2)) || (map_auto_generate_portal_collision(x,z2,ex,ez2,-1))) continue;
		...
		for (k=0;k!=nportal;k++) {
			...
			if ((map_auto_generate_block_collision(x2,z,ex2,ez)) || (map_auto_generate_portal_collision(x2,z,ex2,ez,-1))) continue;
...
void map_auto_generate_lights(void)
{
...
int map_auto_generate_walls_add_segment(int rn,int lx,int lz,int rx,int rz,int ty,int by,int clip,int group_idx,int primitive_uid,int fill,bool movable)
{
	segment_type	*seg;
...
void map_auto_generate_walls_add(int rn,int lx,int lz,int rx,int rz,int ty,int by,int clip,int group_idx,int primitive_uid,int fill)
{
	...
	if (lz==rz) {
		...
		xadd=ag_settings.split_factor-(map.portals[cr].x%ag_settings.split_factor);
		...
		while (rx2!=rx) {
			...
			if (map.nsegment<max_segment) map_auto_generate_walls_add_segment(rn,lx2,lz,rx2,lz,ty,by,clip,group_idx,primitive_uid,fill,FALSE);
		}
	...
	if (lx==rx) {
		...
		zadd=ag_settings.split_factor-(map.portals[cr].z%ag_settings.split_factor);
		...
		while (rz2!=rz) {
			...
			if (map.nsegment<max_segment) map_auto_generate_walls_add_segment(rn,lx,lz2,lx,rz2,ty,by,clip,group_idx,primitive_uid,fill,FALSE);
		}
	...
	if (map.nsegment<max_segment) map_auto_generate_walls_add_segment(rn,lx,lz,rx,rz,ty,by,clip,group_idx,primitive_uid,fill,FALSE);
}

void map_auto_generate_walls(void)
{
	...
	for (n=0;n!=map.nportal;n++) {
		...
		fill=(ag_settings.corridor_flags[n]==ag_corridor_flag_portal)?ag_settings.texture_portal_wall:ag_settings.texture_corridor_wall;
		...
		for (z=portal->z;z!=portal->ez;z++) {
			wall_left_run[z-portal->z]=map_auto_generate_portal_collision(x,z,x,z,n)?1:0;
			wall_right_run[z-portal->z]=map_auto_generate_portal_collision(ex,z,ex,z,n)?1:0;
		}
		...
		for (x=portal->x;x!=portal->ex;x++) {
			wall_top_run[x-portal->x]=map_auto_generate_portal_collision(x,z,x,z,n)?1:0;
			wall_bottom_run[x-portal->x]=map_auto_generate_portal_collision(x,ez,x,ez,n)?1:0;
		}
		...
		for (z=0;z!=zsz;z++) {
			...
			if (wall_left_run[z]==1) {
				z2=z-1;
				if ((in_left_wall) && (lz<z2)) map_auto_generate_walls_add(n,0,lz,0,z2,ty,by,wc_none,-1,-1,fill);
				in_left_wall=FALSE;
			}
			else {
				if (!in_left_wall) {
			...
			if (wall_right_run[z]==1) {
				z2=z-1;
				if ((in_right_wall) && (rz<z2)) map_auto_generate_walls_add(n,xsz,rz,xsz,z2,ty,by,wc_none,-1,-1,fill);
				in_right_wall=FALSE;
			}
			else {
				if (!in_right_wall) {
		...
		if ((in_left_wall) && (lz!=zsz)) map_auto_generate_walls_add(n,0,lz,0,zsz,ty,by,wc_none,-1,-1,fill);
		if ((in_right_wall) && (rz!=zsz)) map_auto_generate_walls_add(n,xsz,rz,xsz,zsz,ty,by,wc_none,-1,-1,fill);
		...
		for (x=0;x!=xsz;x++) {
			...
			if (wall_top_run[x]==1) {
				x2=x-1;
				if ((in_top_wall) && (lx<x2)) map_auto_generate_walls_add(n,lx,0,x2,0,ty,by,wc_none,-1,-1,fill);
				in_top_wall=FALSE;
			}
			else {
				if (!in_top_wall) {
			...
			if (wall_bottom_run[x]==1) {
				x2=x-1;
				if ((in_bottom_wall) && (rx<x2)) map_auto_generate_walls_add(n,rx,zsz,x2,zsz,ty,by,wc_none,-1,-1,fill);
		...
		if ((in_top_wall) && (lx!=xsz)) map_auto_generate_walls_add(n,lx,0,xsz,0,ty,by,wc_none,-1,-1,fill);
		if ((in_bottom_wall) && (rx!=xsz)) map_auto_generate_walls_add(n,rx,zsz,xsz,zsz,ty,by,wc_none,-1,-1,fill);
...
void map_auto_generate_height_walls(void)
{
	...
	for (n=0;n!=map.nportal;n++) {
		...
		for (k=0;k!=map.nportal;k++) {
			...
			if (portal->z==chk_portal->ez) {
				...
				x-=portal->x;
				ex-=portal->x;
				map_auto_generate_walls_add(n,x,0,ex,0,portal->ty,(chk_portal->ty-1),wc_none,-1,-1,fill);
				continue;
			}
			...
			if (portal->ez==chk_portal->z) {
				...
				x-=portal->x;
				ex-=portal->x;
				map_auto_generate_walls_add(n,x,zsz,ex,zsz,portal->ty,(chk_portal->ty-1),wc_none,-1,-1,fill);
				continue;
			}
			...
			if (portal->x==chk_portal->ex) {
				...
				z-=portal->z;
				ez-=portal->z;
				map_auto_generate_walls_add(n,0,z,0,ez,portal->ty,(chk_portal->ty-1),wc_none,-1,-1,fill);
				continue;
			}
			...
			if (portal->ex==chk_portal->x) {
				...
				z-=portal->z;
				ez-=portal->z;
				map_auto_generate_walls_add(n,xsz,z,xsz,ez,portal->ty,(chk_portal->ty-1),wc_none,-1,-1,fill);
				continue;
			}
...
void map_auto_generate_corridor_clip_horizontal_left_above_walls(int rn,int x,int lz,int rz,int ty,int by,int split_factor,int fill_wall)
{
	int				rn2,seg_idx;
	
	seg_idx=map_auto_generate_walls_add_segment(rn,x,(lz+split_factor),x,lz,ty,by,wc_bottom,-1,-1,fill_wall,FALSE);
	if (seg_idx!=-1) {
		rn2=map_auto_generate_portal_find_to_left(&map.portals[rn]);
		map_auto_generate_move_wall_segment_to_portal(rn2,seg_idx);
	}
	
	seg_idx=map_auto_generate_walls_add_segment(rn,x,(rz-split_factor),x,rz,ty,by,wc_bottom,-1,-1,fill_wall,FALSE);
	if (seg_idx!=-1) {
		rn2=map_auto_generate_portal_find_to_left(&map.portals[rn]);
		map_auto_generate_move_wall_segment_to_portal(rn2,seg_idx);
	}
...
void map_auto_generate_corridor_clip_horizontal_right_above_walls(int rn,int x,int lz,int rz,int ty,int by,int split_factor,int fill_wall)
{
	int				rn2,seg_idx;
	
	seg_idx=map_auto_generate_walls_add_segment(rn,x,(lz+split_factor),x,lz,ty,by,wc_bottom,-1,-1,fill_wall,FALSE);
	if (seg_idx!=-1) {
		rn2=map_auto_generate_portal_find_to_right(&map.portals[rn]);
		map_auto_generate_move_wall_segment_to_portal(rn2,seg_idx);
	}
...
void map_auto_generate_corridor_clip_vertical_top_above_walls(int rn,int lx,int rx,int z,int ty,int by,int split_factor,int fill_wall)
{
	int				rn2,seg_idx;
	
	seg_idx=map_auto_generate_walls_add_segment(rn,(lx+split_factor),z,lx,z,ty,by,wc_bottom,-1,-1,fill_wall,FALSE);
	if (seg_idx!=-1) {
		rn2=map_auto_generate_portal_find_to_top(&map.portals[rn]);
		map_auto_generate_move_wall_segment_to_portal(rn2,seg_idx);
	}

	seg_idx=map_auto_generate_walls_add_segment(rn,(rx-split_factor),z,rx,z,ty,by,wc_bottom,-1,-1,fill_wall,FALSE);
	if (seg_idx!=-1) {
		rn2=map_auto_generate_portal_find_to_top(&map.portals[rn]);
		map_auto_generate_move_wall_segment_to_portal(rn2,seg_idx);
	}
...
void map_auto_generate_corridor_clip_vertical_bottom_above_walls(int rn,int lx,int rx,int z,int ty,int by,int split_factor,int fill_wall)
{
	int				rn2,seg_idx;
	
	seg_idx=map_auto_generate_walls_add_segment(rn,(lx+split_factor),z,lx,z,ty,by,wc_bottom,-1,-1,fill_wall,FALSE);
	if (seg_idx!=-1) {
		rn2=map_auto_generate_portal_find_to_bottom(&map.portals[rn]);
		map_auto_generate_move_wall_segment_to_portal(rn2,seg_idx);
	}
...
void map_auto_generate_corridor_clip_horizontal_left_below_walls(int rn,int x,int lz,int rz,int ty,int by,int split_factor,int fill_wall)
{
	int				rn2,seg_idx;
	
	seg_idx=map_auto_generate_walls_add_segment(rn,x,lz,x,(lz+split_factor),ty,by,wc_top,-1,-1,fill_wall,FALSE);
	if (seg_idx!=-1) {
		rn2=map_auto_generate_portal_find_to_left(&map.portals[rn]);
		map_auto_generate_move_wall_segment_to_portal(rn2,seg_idx);
	}
	
	seg_idx=map_auto_generate_walls_add_segment(rn,x,rz,x,(rz-split_factor),ty,by,wc_top,-1,-1,fill_wall,FALSE);
	if (seg_idx!=-1) {
		rn2=map_auto_generate_portal_find_to_left(&map.portals[rn]);
		map_auto_generate_move_wall_segment_to_portal(rn2,seg_idx);
	}
...
void map_auto_generate_corridor_clip_horizontal_right_below_walls(int rn,int x,int lz,int rz,int ty,int by,int split_factor,int fill_wall)
{
	int				rn2,seg_idx;
...
void map_auto_generate_corridor_clip_vertical_top_below_walls(int rn,int lx,int rx,int z,int ty,int by,int split_factor,int fill_wall)
{
	int				rn2,seg_idx;
	
	seg_idx=map_auto_generate_walls_add_segment(rn,lx,z,(lx+split_factor),z,ty,by,wc_top,-1,-1,fill_wall,FALSE);
	if (seg_idx!=-1) {
		rn2=map_auto_generate_portal_find_to_top(&map.portals[rn]);
		map_auto_generate_move_wall_segment_to_portal(rn2,seg_idx);
	}

	seg_idx=map_auto_generate_walls_add_segment(rn,rx,z,(rx-split_factor),z,ty,by,wc_top,-1,-1,fill_wall,FALSE);
	if (seg_idx!=-1) {
		rn2=map_auto_generate_portal_find_to_top(&map.portals[rn]);
		map_auto_generate_move_wall_segment_to_portal(rn2,seg_idx);
	}
...
void map_auto_generate_corridor_clip_vertical_bottom_below_walls(int rn,int lx,int rx,int z,int ty,int by,int split_factor,int fill_wall)
{
	int				rn2,seg_idx;
...
void map_auto_generate_corridor_clip_walls(void)
{
	...
	for (n=0;n!=map.nportal;n++) {
		...
		if ((ag_settings.corridor_type==ag_corridor_type_slanted_ceiling) || (ag_settings.corridor_type==ag_corridor_type_octagon)) {
			...
			if (ag_settings.corridor_flags[n]==ag_corridor_flag_horizontal) {
				map_auto_generate_corridor_clip_horizontal_left_above_walls(n,lx,lz,rz,ty,by,ag_settings.split_factor,ag_settings.texture_portal_wall);
				map_auto_generate_corridor_clip_horizontal_right_above_walls(n,rx,lz,rz,ty,by,ag_settings.split_factor,ag_settings.texture_portal_wall);
			}
			if (ag_settings.corridor_flags[n]==ag_corridor_flag_vertical) {
				map_auto_generate_corridor_clip_vertical_top_above_walls(n,lx,rx,lz,ty,by,ag_settings.split_factor,ag_settings.texture_portal_wall);
				map_auto_generate_corridor_clip_vertical_bottom_above_walls(n,lx,rx,rz,ty,by,ag_settings.split_factor,ag_settings.texture_portal_wall);
			}
		...
		if (ag_settings.corridor_type==ag_corridor_type_octagon) {
			...
			if (ag_settings.corridor_flags[n]==ag_corridor_flag_horizontal) {
				map_auto_generate_corridor_clip_horizontal_left_below_walls(n,lx,lz,rz,ty,by,ag_settings.split_factor,ag_settings.texture_portal_wall);
				map_auto_generate_corridor_clip_horizontal_right_below_walls(n,rx,lz,rz,ty,by,ag_settings.split_factor,ag_settings.texture_portal_wall);
			}
			if (ag_settings.corridor_flags[n]==ag_corridor_flag_vertical) {
				map_auto_generate_corridor_clip_vertical_top_below_walls(n,lx,rx,lz,ty,by,ag_settings.split_factor,ag_settings.texture_portal_wall);
				map_auto_generate_corridor_clip_vertical_bottom_below_walls(n,lx,rx,rz,ty,by,ag_settings.split_factor,ag_settings.texture_portal_wall);
			}
...
void map_auto_generate_fc_add_segment(int rn,int type,int lx,int lz,int rx,int rz,int y,int group_idx,int primitive_uid,int fill,bool movable,int lower_mode)
{
	segment_type	*seg;
...
void map_auto_generate_fc_add(int rn,int lx,int lz,int rx,int rz,int ty,int by)
{
	int				xoff,zoff,
	...
	fill_floor=is_portal?ag_settings.texture_portal_floor:ag_settings.texture_corridor_floor;
	fill_ceiling=is_portal?ag_settings.texture_portal_ceiling:ag_settings.texture_corridor_ceiling;

	fill_wall=ag_settings.texture_portal_wall;
	...
	while (rz2!=rz) {
		...
		while (rx2!=rx) {
			...
			if (ag_settings.floors) {
				...
				if (is_portal) {
					map_auto_generate_fc_add_segment(rn,sg_floor,lx2,lz2,rx2,rz2,by,-1,-1,fill_floor,FALSE,ag_ceiling_lower_none);
				}
					...
					map_auto_generate_fc_add_segment(rn,sg_floor,lx2,lz2,rx2,rz2,by,-1,-1,fill_floor,FALSE,lower_mode);
			...
			if (ag_settings.ceilings) {
				...
				if (is_portal) {
					...
					map_auto_generate_fc_add_segment(rn,sg_ceiling,lx2,lz2,rx2,rz2,ty,-1,-1,fill_ceiling,FALSE,ag_ceiling_lower_none);
				}
				else {
					...
					map_auto_generate_fc_add_segment(rn,sg_ceiling,lx2,lz2,rx2,rz2,ty,-1,-1,fill_ceiling,FALSE,lower_mode);
				}
			}
...
void map_auto_generate_fcs(void)
{
	int				n,xsz,zsz;
	portal_type		*portal;
	...
	for (n=0;n!=map.nportal;n++) {
		...
		map_auto_generate_fc_add(n,0,0,xsz,zsz,portal->ty,portal->by);
...
void map_auto_generate_open_ceiling_walls(void)
{
	...
	for (n=0;n!=map.nportal;n++) {
		...
		fill=ag_settings.texture_portal_ceiling;
		...
		map_auto_generate_walls_add(n,lx,lz,rx,lz,ty,by,wc_none,-1,-1,fill);
		map_auto_generate_walls_add(n,lx,rz,rx,rz,ty,by,wc_none,-1,-1,fill);
		map_auto_generate_walls_add(n,lx,lz,lx,rz,ty,by,wc_none,-1,-1,fill);
		map_auto_generate_walls_add(n,rx,lz,rx,rz,ty,by,wc_none,-1,-1,fill);
...
int map_auto_generate_rough_floor_find_floor(bool portal_only)
{
	int				idx;
...
void map_auto_generate_rough_floor(void)
{
	...
	for (n=0;n!=nvertexshift;n++) {
		seg_idx=map_auto_generate_rough_floor_find_floor(portal_only);
		vertex_idx=rand()%4;
...
void map_auto_generate_doors(void)
{
	...
	for (n=0;n!=map.nportal;n++) {
		...
		if (ag_settings.corridor_flags[n]==ag_corridor_flag_horizontal) {
			...
			map_auto_generate_walls_add_segment(n,x,0,x,zsz,ty,by,wc_none,group_idx,primitive_uid,ag_settings.texture_door,TRUE);
			map_auto_generate_walls_add_segment(n,(x+2),0,(x+2),zsz,ty,by,wc_none,group_idx,primitive_uid,ag_settings.texture_door,TRUE);
			map_auto_generate_fc_add_segment(n,sg_ceiling,x,0,(x+2),zsz,ty,group_idx,primitive_uid,ag_settings.texture_door,TRUE,ag_ceiling_lower_none);
			map_auto_generate_fc_add_segment(n,sg_floor,x,0,(x+2),zsz,(by+1),group_idx,primitive_uid,ag_settings.texture_door,TRUE,ag_ceiling_lower_none);
		}
		else {
			...
			map_auto_generate_walls_add_segment(n,0,z,xsz,z,ty,by,wc_none,group_idx,primitive_uid,ag_settings.texture_door,TRUE);
			map_auto_generate_walls_add_segment(n,0,(z+2),xsz,(z+2),ty,by,wc_none,group_idx,primitive_uid,ag_settings.texture_door,TRUE);
			map_auto_generate_fc_add_segment(n,sg_ceiling,0,z,xsz,(z+2),ty,group_idx,primitive_uid,ag_settings.texture_door,TRUE,ag_ceiling_lower_none);
			map_auto_generate_fc_add_segment(n,sg_floor,0,z,xsz,(z+2),(by+1),group_idx,primitive_uid,ag_settings.texture_door,TRUE,ag_ceiling_lower_none);
		}
...
void map_auto_generate_spots(void)
{
	int				n,k,x,y,z,idx;
	segment_type	*seg;
	spot_type		*spot;
	...
	for (n=0;n!=ag_settings.spot_count;n++) {
		idx=map_auto_generate_rough_floor_find_floor(TRUE);
...
void map_import_auto_generate(void)
{
		// clear the map
	...
	map_auto_generate_initial_portals();
	map_auto_generate_merge_portals();
	map_auto_generate_connect_portals();
	map_auto_generate_portal_y();
	
		// create lights
		
	map_auto_generate_lights();
	
		// create segments

	map_auto_generate_walls();
	map_auto_generate_height_walls();
	map_auto_generate_corridor_clip_walls();
	map_auto_generate_fcs();
	map_auto_generate_open_ceiling_walls();
	map_auto_generate_rough_floor();
	
		// create doors
		
	map_auto_generate_doors();
