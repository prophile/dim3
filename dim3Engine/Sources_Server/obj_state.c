...
void object_death(obj_type *obj)
{
	...
	if ((net_setup.client.joined) && (obj->uid==server.player_obj_uid)) network_client_send_death(net_setup.client.remote_uid,obj->damage_obj_uid);
}
...
void object_telefrag_check(obj_type *obj)
{
	...
	for (n=0;n!=server.count.obj;n++) {
		if ((obj->uid==check_obj->uid) || (!check_obj->remote.on)) {
			check_obj++;
			continue;
		}
	...
	if (net_setup.client.joined) network_client_send_telefrag(net_setup.client.remote_uid,hit_obj->uid);
}
...
void object_setup_hit(obj_type *obj,obj_type *from_obj,weapon_type *from_weap,proj_type *from_proj,d3pnt *melee_hit_pt)
{
	...
	if ((obj->hit_box.on) && (obj->hit_box.hit)) {
		model=model_find_uid(obj->hit_box.model_uid);
