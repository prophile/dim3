...
int view_images_load_single(char *path,bool simple)
{
	...
	if (simple) {
		bitmap_open(&image->bitmap,path,anisotropic_mode_none,texture_quality_mode_high,mipmap_mode_none,FALSE,FALSE);
	}
	else {
		bitmap_open(&image->bitmap,path,setup.anisotropic_mode,setup.texture_quality_mode,setup.mipmap_mode,setup.mipmap_card_generated,setup.texture_compression);
	}
