...
void walk_view_models_reset(void)
{
	...
	for (n=0;n!=map.nspot;n++) {
		...
		if (spot->display_model[0]!=0x0) {
			...
			if ((idx==-1) && (nmodel<max_model)) {
				if (model_open(&models[nmodel],spot->display_model,0,TRUE,FALSE,FALSE)) nmodel++;
			}
		}
	...
	for (n=0;n!=map.nscenery;n++) {
		...
		if (scenery->model_name[0]!=0x0) {
			...
			if ((idx==-1) && (nmodel<max_model)) {
				if (model_open(&models[nmodel],scenery->model_name,0,TRUE,FALSE,FALSE)) nmodel++;
			}
		}
...
bool walk_view_model_draw(d3pos *pos,d3ang *ang,char *name)
{
    ...
    draw_setup.pose_1=draw_setup.pose_2=-1;
    draw_setup.pose_factor=0;
	draw_setup.pose_acceleration=0;
