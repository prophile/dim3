...
#define ctrl_network_timeout_id				62
#define ctrl_network_show_names_id			63
#define ctrl_network_host_id				64
#define ctrl_network_host_ip_id				65
...
#define setup_network_host_add_button		92
#define setup_network_host_update_button	93
#define setup_network_host_delete_button	94

#define setup_ok_button						100
#define setup_cancel_button					101
#define setup_default_button				102
...
int							setup_tab_value,setup_action_scroll_pos,setup_network_host_scroll_pos,
...
char						setup_screen_size_list[max_screen_size][32],
							...
							setup_team_color_list[][32]=net_team_color_list_def,
...
bool						setup_start_trigger,setup_action_set_flag,setup_action_set_last_click,
							setup_in_game;
...
void setup_action_pane(void)
{
	...
	y=((y+high)+(setup.screen.y_scale-60))>>1;
	...
	element_button_add(path,path2,setup_action_set_button,(x-5),y,-1,-1,element_pos_right,element_pos_center);
	...
	element_button_add(path,path2,setup_action_clear_button,(x+5),y,-1,-1,element_pos_left,element_pos_center);
	element_enable(setup_action_clear_button,FALSE);
}

void setup_network_pane(void)
{
	int							n,x,y,wid,high,control_y_add;
	char						path[1024],path2[1024];
...
void setup_create_pane(void)
{
	int			x,y,n,high,ntab,stab,pane;
	char		path[1024],path2[1024],
				tab_path[6][1024],tab_path2[6][1024];
	char		tab_list[][32]={"tab_display","tab_graphics","tab_audio","tab_mouse","tab_actions","tab_network"},
				tab_selected_list[][32]={"tab_display_selected","tab_graphics_selected","tab_audio_selected","tab_mouse_selected","tab_actions_selected","tab_network_selected"};
	...
	element_button_add(path,path2,setup_default_button,x,y,-1,-1,element_pos_left,element_pos_center);
	...
	element_button_add(path,path2,setup_ok_button,x,y,-1,-1,element_pos_right,element_pos_center);

	x=element_get_x_position(setup_ok_button)-10;
	...
	element_button_add(path,path2,setup_cancel_button,x,y,-1,-1,element_pos_right,element_pos_center);
...
int setup_action_find_free_attach(setup_action_type *action)
{
	int					n;
...
void setup_action_enable_buttons(void)
{
	...
	if (setup_action_find_free_attach(action)!=-1) element_enable(setup_action_set_button,TRUE);
}

void setup_action_clear(void)
{
	...
	setup_action_enable_buttons();
}

void setup_action_wait_for_input(void)
{
	int					n,ctrl_idx,action_idx;
	char				name[32];
	bool				no_key_up;
	...
	n=setup_action_find_free_attach(action);
...
void setup_open(bool in_game)
{
	...
	server.state=gs_setup;
}

void setup_cancel_close(void)
{
...
void setup_trigger_clear(void)
{
	setup_start_trigger=FALSE;
}

void setup_trigger_check(void)
{
	if (setup_start_trigger) setup_open(TRUE);			// trigger only checked when game running
}	

void setup_trigger_set(void)
{
	setup_start_trigger=TRUE;
}
...
void setup_handle_click(int id)
{
	int			idx;
	...
	switch (id) {
		...
		case setup_cancel_button:
			setup_cancel_close();
			return;

		case setup_default_button:
			setup_default();
			return;
...
void setup_keyboard(void)
{
	...
	setup_handle_click(id);
}

void setup_click(void)
{
	int			id;
